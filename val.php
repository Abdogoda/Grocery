        ...
        if(isset($_POST['buy'])){
          ...
          while($rowbag = mysqli_fetch_array($allbag)){
            ...
            while($row1 = mysqli_fetch_array($result1)){
              if($row1['category'] == "meat" || $row1['category'] =="dairy" || $row1['category'] =="fruits" || $row1['category'] =="vegetables"){
                  $bod .= "
                      <tr>
                          <td>$row1[name]</td>
                          <td>$row1[category]</td>
          ...
          // Reset mybag
            mysqli_query($con, "DELETE FROM mybag");
            echo "<script>alert('The Process is made Successfully!');</script>";
        }
    }
        ?>
    <center>
        ...
        <br>
        <div class="total fs-3 bg-secondary rounded-5"> Total:  <span><?php echo "$col[0]$";?></span></div>
        <br><br>
            <button onclick="window.print()" class="btn btn-primary text-white fs-3 d-inline">Print</button>
            <form action="val.php" method="POST" class="d-inline">
              <button name="buy" class="btn btn-success text-white fs-3">Buy</button>
            </form><br><br><br>
        <a href="mybag.php" class="btn btn-info text-white fw-bold float-end p-1">Back To MyBag >></a>
